...
def dashboard_bar_smooth(long_df, phases, params):
        ...
        if multi_patient:
            ...
            if one_phase:
                ...
                fig.update_xaxes(
                    ...
                    tickvals=list(x_num), ticktext=labels, tickangle=-45,
                    automargin=True, row=rr, col=cc
                )
            else:
    ...
    fig.update_layout(
        ...
        margin=dict(l=55, r=40, t=80, b=150),
...
st.caption("v15.5.8 · Atractores animados en pestaña 4 · Persistent Cloud Architecture II · Narrativa predictiva Presente → Tendencia → Probabilidad · Control Autonómico doble plano V/S/L/C/R · Nivel 2 XGBoost")
...
def _autonomic_abs_vector_levels(row):
    """Nivel actual V/S/L/C/R sin usar información futura.

    Se apoya principalmente en los índices fisiológicos 0–100 ya existentes para evitar
    imponer umbrales absolutos universales a RMSSD/SDNN/etc. Añade evidencia contextual
    de dominancias espectrales y DFA solo como apoyo, no como sustituto del índice.
    """
    ...
    S=_autonomic_abs_from_index(get('IDX_Rigidez'))

    # Evidencia absoluta contextual: nunca interpreta LF/LF-HF como simpático puro.
    hf=get('HF_DOM_PCT'); lf=get('LF_DOM_PCT'); dfa1=get('DFA_alpha1')
    if np.isfinite(hf) and np.isfinite(lf):
        vagal_dom=float(np.clip((hf-lf)/25.0,-1.5,1.5))
        V=_autonomic_weighted_score([(V,0.80),(vagal_dom,0.20)])
        alert_dom=float(np.clip((lf-hf)/25.0,-1.5,1.5))
        S=_autonomic_weighted_score([(S,0.75),(alert_dom,0.25)])
    if np.isfinite(dfa1):
        # Sólo apoyo de organización rápida: máximo apoyo alrededor/por encima de 1.15;
        # valores muy bajos no se convierten en "bajo simpático", se tratan aparte como desacoplamiento.
        dfa_alert=float(np.clip((dfa1-1.0)/0.30,-1.0,1.5)) if dfa1>=0.5 else np.nan
        S=_autonomic_weighted_score([(S,0.80),(dfa_alert,0.20)])
...
def autonomic_control_assessment(df, row, record_data=None):
    """v15.5.6 · Matriz V/S/L/C/R/Q con doble plano: nivel actual + cambio longitudinal.

    La clasificación A–F ya no confunde "estable respecto al basal" con "nivel bajo".
    El histórico informa tendencia; el nivel actual conserva la magnitud fisiológica.
    """
    ...
    # Plano longitudinal: cambio respecto al propio histórico previo.
    ...
    # Plano absoluto/actual: preserva la magnitud aunque el paciente sea estable longitudinalmente.
    abs_levels=_autonomic_abs_vector_levels(row)
    V_abs,S_abs,L_abs,C_abs,R_abs=(abs_levels[k] for k in ['V','S','L','C','R'])
    V=_autonomic_fuse_planes(V_abs,V_long)
    S=_autonomic_fuse_planes(S_abs,S_long)
    L=_autonomic_fuse_planes(L_abs,L_long)
    Cvec=_autonomic_fuse_planes(C_abs,C_long)
    R=_autonomic_fuse_planes(R_abs,R_long)
    ...
    cycle_limit='ciclo límite' in attr.lower(); fixed_like='punto fijo' in attr.lower()

    # Reglas A–F: usan el nivel efectivo (actual + longitudinal).
    rules=[]
    ...
    B=(np.isfinite(V) and V>0.35) and (np.isfinite(S) and S>0.30) and ((np.isfinite(dfa1) and dfa1>1.15) or (np.isfinite(lfdom) and lfdom>40)) and (np.isfinite(lfdom) and np.isfinite(hfdom) and lfdom>hfdom)
    rules.append(('Coactivación con predominio simpático',B))
    Cc=(np.isfinite(V) and V>0.50) and ((np.isfinite(S) and S>-0.10) or (np.isfinite(lfdom) and lfdom>25)) and ((np.isfinite(lf_hf) and lf_hf<1.5) or (np.isfinite(hfdom) and np.isfinite(lfdom) and hfdom>lfdom)) and (not A)
    rules.append(('Coactivación con predominio parasimpático',Cc))
    D=(np.isfinite(V) and V>0.45) and ((np.isfinite(S) and S<0.0) or (np.isfinite(dfa1) and dfa1<=1.0)) and (rigid_low or (np.isfinite(S_long) and S_long<0)) and (not A)
    rules.append(('Reciprocidad · predominio vagal puro',D))
    E=(np.isfinite(V) and V<-0.55) and ((np.isfinite(S) and S>0.45) or rigid_high) and (cycle_limit or (rigid_high and np.isfinite(S) and S>0.70))
    ...
    if matched:
        ...
        scores={
            ...
            'Reciprocidad · predominio vagal puro': max(0,V if np.isfinite(V) else 0)+max(0,-S if np.isfinite(S) else 0)+float(np.isfinite(dfa1) and dfa1<=1.0)+float(np.isfinite(S_long) and S_long<0)+float(rigid_low),
        ...
        base_state=max(scores,key=scores.get)

    # Descriptor jerárquico: forma + predominio + amplitud/reserva + organización.
    amp_label='alta amplitud' if (np.isfinite(R) and R>=0.50) else ('baja amplitud' if (np.isfinite(R) and R<=-0.50) else 'amplitud intermedia')
    if np.isfinite(V) and np.isfinite(S):
        predominio='vagal' if V>=S+0.20 else ('alerta/barorreflejo' if S>=V+0.20 else 'equilibrado')
    else:
        predominio='vagal' if np.isfinite(V) and V>0.4 else ('alerta/barorreflejo' if np.isfinite(S) and S>0.4 else 'indeterminado')
    if np.isfinite(Cvec):
        dyn_label='flexible/coherente' if Cvec>=0.25 and not A else ('rígida/poco móvil' if Cvec<=-0.35 else 'intermedia')
    else:
        dyn_label='no determinada'

    display_state=base_state
    if base_state=='Coactivación con predominio parasimpático':
        display_state=('Coactivación autonómica de alta amplitud con predominio vagal' if amp_label=='alta amplitud'
                       else 'Coactivación autonómica con predominio vagal')
    elif base_state=='Coactivación con predominio simpático':
        display_state=('Coactivación autonómica de alta amplitud con predominio de alerta/barorreflejo' if amp_label=='alta amplitud'
                       else 'Coactivación autonómica con predominio de alerta/barorreflejo')
    ...
    def add(k,label=None):
        ...
        if np.isfinite(v): reason.append(f"{label or k}={v:.3g}"+(f" (Δ/Z={zz:+.2f})" if np.isfinite(zz) else ""))
    for k in ['RMSSD','SD1','pNN50','HF_DOM_PCT','DFA_alpha1','LF_DOM_PCT','LF_HF','MeanHR','IDX_Rigidez','VLF_DOM_PCT','DFA_alpha2','Hurst','SampEn','DispEn','D2','Lyapunov_LLE','WAV_ENTROPY_GLOBAL','WAV_TRANSITIONS_PER_MIN','HVG_graph_score_small_world','HVG_clustering','SDNN','SD2','TOTAL','IDX_Vagal','IDX_Amplitud','IDX_Complejidad']:
        add(k)
    reason.append(f"Plano actual V/S/L/C/R={V_abs:+.2f}/{S_abs:+.2f}/{L_abs:+.2f}/{C_abs:+.2f}/{R_abs:+.2f}" if all(np.isfinite(x) for x in [V_abs,S_abs,L_abs,C_abs,R_abs]) else "Plano actual parcialmente disponible")
    if any(np.isfinite(x) for x in [V_long,S_long,L_long,C_long,R_long]):
        reason.append("Plano longitudinal V/S/L/C/R="+'/'.join(f"{x:+.2f}" if np.isfinite(x) else 'N/D' for x in [V_long,S_long,L_long,C_long,R_long]))
    reason.append(f"Predominio={predominio}; {amp_label}; dinámica={dyn_label}")
    if np.isfinite(hf_cons): reason.append(f"HF_consenso_Δ/Z={hf_cons:+.2f}")
    if np.isfinite(lf_cons): reason.append(f"LF_consenso_Δ/Z={lf_cons:+.2f}")
    if np.isfinite(vlf_cons): reason.append(f"VLF_consenso_Δ/Z={vlf_cons:+.2f}")
    if np.isfinite(total_cons): reason.append(f"TOTAL_consenso_Δ/Z={total_cons:+.2f}")
    reason.append(f"Atractor={attr}")

    spec=AUTONOMIC_FORMS[base_state]
    return {
        'Estado_control_autonomico':display_state,'Forma_base_A_F':base_state,
        'Predominio_autonomico':predominio,'Amplitud_reserva':amp_label,'Organizacion_dinamica':dyn_label,
        'Regla_estricta_cumplida':exact,'Confianza_clasificacion_pct':conf,
        ...
        'Vector_V_Efectivo':V,'Vector_S_Efectivo':S,'Vector_L_Efectivo':L,'Vector_C_Efectivo':Cvec,'Vector_R_Efectivo':R,
        'Calidad_Q':Q,
        ...
        'Referencia_n':int(len(ref)),'Referencia_metodo':_autonomic_reference_method(len(ref)),'Referencia_fase':str(row.get('Fase','')),
    }
...
with tab10:
    st.subheader("Control autonómico · doble plano: nivel actual + evolución longitudinal V/S/L/C/R")
    st.info(
    ...
    if _auto_table_v1554.empty:
        ...
        st.markdown("### Perfil autonómico actual + cambio longitudinal")
        st.caption("La clasificación combina el **nivel fisiológico actual** con el **cambio respecto al histórico previo**. Un valor estable longitudinalmente no borra un nivel actual alto.")
        _abs_df_v1556=pd.DataFrame({
            'Vector':['Vagal (V)','Alerta/barorreflejo (S)','Regulación lenta (L)','Complejidad/movilidad (C)','Reserva/amplitud (R)'],
            'Nivel_actual':[_ar.get('Vector_V_Actual',np.nan),_ar.get('Vector_S_Actual',np.nan),_ar.get('Vector_L_Actual',np.nan),_ar.get('Vector_C_Actual',np.nan),_ar.get('Vector_R_Actual',np.nan)],
            'Cambio_longitudinal':[_ar.get('Vector_V_Z',np.nan),_ar.get('Vector_S_Z',np.nan),_ar.get('Vector_L_Z',np.nan),_ar.get('Vector_C_Z',np.nan),_ar.get('Vector_R_Z',np.nan)],
            'Nivel_efectivo':[_ar.get('Vector_V_Efectivo',np.nan),_ar.get('Vector_S_Efectivo',np.nan),_ar.get('Vector_L_Efectivo',np.nan),_ar.get('Vector_C_Efectivo',np.nan),_ar.get('Vector_R_Efectivo',np.nan)]
        })
        st.dataframe(_abs_df_v1556,use_container_width=True,hide_index=True)
        _d1,_d2,_d3=st.columns(3)
        _d1.metric("Predominio",str(_ar.get('Predominio_autonomico','N/D')))
        _d2.metric("Amplitud / reserva",str(_ar.get('Amplitud_reserva','N/D')))
        _d3.metric("Organización dinámica",str(_ar.get('Organizacion_dinamica','N/D')))
        ...
        _show_cols=['Fecha_hora','Registro','Fase','Estado_control_autonomico','Regla_estricta_cumplida','Confianza_clasificacion_pct',
                    'Referencia_n','Referencia_metodo','Forma_base_A_F','Predominio_autonomico','Amplitud_reserva','Organizacion_dinamica','Vector_V_Actual','Vector_S_Actual','Vector_L_Actual','Vector_C_Actual','Vector_R_Actual','Vector_V_Z','Vector_S_Z','Vector_L_Z','Vector_C_Z','Vector_R_Z','Vector_V_Efectivo','Vector_S_Efectivo','Vector_L_Efectivo','Vector_C_Efectivo','Vector_R_Efectivo','Calidad_Q','Coste_alostatico']
        ...
        st.download_button(
            ...
            file_name='control_autonomico_v1556.csv',mime='text/csv'
        )
